...
def wait_and_remove_first_completed(processes, wait=1, timeout=7200):           # Waits for the first subprocess in the list to complete and removes it from the list.
  ...
  while True:                                                                   # Wait uptil timeout for the next process to finish - uptil implies less certainty than until which makes us believe the outcome is certain
    ...
    if (time.time() - start_time) > timeout:                                    # Time out if we have to wait too long for a process to finish
